...
template <typename T>
ez::matN<T> makeSigma(const ez::vecN<T>& aSingularValues) {
...
template <typename T>
ez::matN<T> reconstruct(const ez::svd::result<T>& aResult) {
...
bool TestEzSvd_EmptyMatrix() {
    ez::matN<T> empty;
    ez::svd::solver<T> svdSolver;
    ez::svd::result<T> output = svdSolver.compute(empty);
...
bool TestEzSvd_Identity3x3() {
    ...
    ez::svd::solver<T> svdSolver;
    ez::svd::result<T> output = svdSolver.compute(identity);
    CTEST_ASSERT(output.success);
    CTEST_ASSERT(output.s.size() == 3);
    CTEST_ASSERT(ez::isEqual(output.s[0], static_cast<T>(1), static_cast<T>(1e-6)));
    CTEST_ASSERT(ez::isEqual(output.s[1], static_cast<T>(1), static_cast<T>(1e-6)));
    CTEST_ASSERT(ez::isEqual(output.s[2], static_cast<T>(1), static_cast<T>(1e-6)));
    return true;
}
...
bool TestEzSvd_Diagonal() {
    ...
    ez::svd::solver<T> svdSolver;
    ez::svd::result<T> output = svdSolver.compute(diagonal);
    CTEST_ASSERT(output.success);
    CTEST_ASSERT(output.s.size() == 3);
    CTEST_ASSERT(ez::isEqual(output.s[0], static_cast<T>(3), static_cast<T>(1e-6)));
    CTEST_ASSERT(ez::isEqual(output.s[1], static_cast<T>(2), static_cast<T>(1e-6)));
    CTEST_ASSERT(ez::isEqual(output.s[2], static_cast<T>(1), static_cast<T>(1e-6)));
    return true;
}
...
bool TestEzSvd_ReconstructionSquare() {
    ...
    ez::svd::solver<T> svdSolver;
    ez::svd::result<T> output = svdSolver.compute(matrix);
    CTEST_ASSERT(output.success);
    CTEST_ASSERT(output.s.size() == 3);
