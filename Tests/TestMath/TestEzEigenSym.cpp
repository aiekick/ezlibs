...
template <typename T>
ez::matN<T> makeDiagonal(const ez::vecN<T>& aValues) {
    std::size_t n = aValues.size();
    ez::matN<T> diagonal(n, n);
...
template <typename T>
bool matrixIsEqual(const ez::matN<T>& aLeft, const ez::matN<T>& aRight, T aTolerance) {
    ...
    for (std::size_t i = 0; i < aLeft.rows(); ++i) {
        for (std::size_t j = 0; j < aLeft.columns(); ++j) {
            if (!ez::isEqual(aLeft(i, j), aRight(i, j), aTolerance)) {
                return false;
            }
        }
    }
...
template <typename T>
ez::matN<T> reconstruct(const ez::eigenSym::result<T>& aResult) {
...
bool TestEzEigenSym_EmptyMatrix() {
    ez::matN<T> empty;
    ez::eigenSym::solver<T> eigSolver;
    ez::eigenSym::result<T> output = eigSolver.compute(empty);
...
bool TestEzEigenSym_NonSquareMatrix() {
    ez::matN<T> rectangular(3, 4);
    ez::eigenSym::solver<T> eigSolver;
    ez::eigenSym::result<T> output = eigSolver.compute(rectangular);
    CTEST_ASSERT(!output.success);
    return true;
}
...
bool TestEzEigenSym_Identity3x3() {
    ez::matN<T> identity = ez::matN<T>::Identity(3);
    ez::eigenSym::solver<T> eigSolver;
    ez::eigenSym::result<T> output = eigSolver.compute(identity);
    CTEST_ASSERT(output.success);
    CTEST_ASSERT(output.d.size() == 3);
    CTEST_ASSERT(ez::isEqual(output.d[0], static_cast<T>(1), static_cast<T>(1e-6)));
    CTEST_ASSERT(ez::isEqual(output.d[1], static_cast<T>(1), static_cast<T>(1e-6)));
    CTEST_ASSERT(ez::isEqual(output.d[2], static_cast<T>(1), static_cast<T>(1e-6)));
    return true;
}
...
bool TestEzEigenSym_Diagonal() {
    ...
    ez::matN<T> diagonal(3, 3);
    diagonal(0, 0) = static_cast<T>(3);
    diagonal(1, 1) = static_cast<T>(1);
    diagonal(2, 2) = static_cast<T>(2);
    ez::eigenSym::solver<T> eigSolver;
    ez::eigenSym::result<T> output = eigSolver.compute(diagonal);
    CTEST_ASSERT(output.success);
    CTEST_ASSERT(ez::isEqual(output.d[0], static_cast<T>(3), static_cast<T>(1e-6)));
    CTEST_ASSERT(ez::isEqual(output.d[1], static_cast<T>(2), static_cast<T>(1e-6)));
    CTEST_ASSERT(ez::isEqual(output.d[2], static_cast<T>(1), static_cast<T>(1e-6)));
    return true;
}
...
bool TestEzEigenSym_DiagonalWithNegative() {
    ...
    ez::eigenSym::result<T> output = eigSolver.compute(diagonal);
    CTEST_ASSERT(output.success);
    CTEST_ASSERT(ez::isEqual(output.d[0], static_cast<T>(5), static_cast<T>(1e-6)));
    CTEST_ASSERT(ez::isEqual(output.d[1], static_cast<T>(1), static_cast<T>(1e-6)));
    CTEST_ASSERT(ez::isEqual(output.d[2], static_cast<T>(-2), static_cast<T>(1e-6)));
    return true;
}
...
bool TestEzEigenSym_ReconstructionSymmetric3x3() {
    ...
    ez::matN<T> reconstructed = reconstruct(output);
...
bool TestEzEigenSym_EigenvectorsAreOrthogonal() {
    ...
    ez::matN<T> product = output.v * output.v.transpose();
    ez::matN<T> identity = ez::matN<T>::Identity(3);
