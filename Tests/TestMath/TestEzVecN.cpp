#include <ezlibs/ezMath/ezVecN.hpp>
...
bool TestEzVecN_InitializerListConstructor() {
    ez::math::vecN<T> vector{static_cast<T>(1), static_cast<T>(2), static_cast<T>(3)};
    CTEST_ASSERT(vector.size() == 3);
    CTEST_ASSERT(ez::math::isEqual(vector[0], static_cast<T>(1)));
    CTEST_ASSERT(ez::math::isEqual(vector[1], static_cast<T>(2)));
    CTEST_ASSERT(ez::math::isEqual(vector[2], static_cast<T>(3)));
    return true;
}
...
bool TestEzVecN_AccessAndAssign() {
    ez::math::vecN<T> vector(3);
    vector[0] = static_cast<T>(10);
    vector[1] = static_cast<T>(20);
    vector[2] = static_cast<T>(30);
    CTEST_ASSERT(ez::math::isEqual(vector[0], static_cast<T>(10)));
    CTEST_ASSERT(ez::math::isEqual(vector[1], static_cast<T>(20)));
    CTEST_ASSERT(ez::math::isEqual(vector[2], static_cast<T>(30)));
    return true;
}

template <typename T>
bool TestEzVecN_LengthSquared() {
    ez::math::vecN<T> vector{static_cast<T>(3), static_cast<T>(4)};
    CTEST_ASSERT(ez::math::isEqual(vector.lengthSquared(), static_cast<T>(25)));
    return true;
}
...
bool TestEzVecN_Length() {
    ez::math::vecN<T> vector{static_cast<T>(3), static_cast<T>(4)};
    CTEST_ASSERT(ez::math::isEqual(vector.length(), static_cast<T>(5)));
    return true;
}

template <typename T>
bool TestEzVecN_DotProduct() {
    ez::math::vecN<T> left{static_cast<T>(1), static_cast<T>(2), static_cast<T>(3)};
    ez::math::vecN<T> right{static_cast<T>(4), static_cast<T>(5), static_cast<T>(6)};
    // 1*4 + 2*5 + 3*6 = 32
    CTEST_ASSERT(ez::math::isEqual(left.dot(right), static_cast<T>(32)));
    return true;
}

template <typename T>
bool TestEzVecN_DotProductDimensionMismatch() {
    ez::math::vecN<T> left(3, static_cast<T>(1));
    ez::math::vecN<T> right(4, static_cast<T>(1));
    // Mismatched sizes return 0
    CTEST_ASSERT(ez::math::isEqual(left.dot(right), static_cast<T>(0)));
    return true;
}
...
bool TestEzVecN_Normalize() {
    ...
    CTEST_ASSERT(ez::math::isEqual(vector.length(), static_cast<T>(1), static_cast<T>(1e-6)));
    return true;
}

template <typename T>
bool TestEzVecN_NormalizeNullVector() {
    ez::math::vecN<T> vector{static_cast<T>(0), static_cast<T>(0), static_cast<T>(0)};
    bool normalized = vector.normalize();
    CTEST_ASSERT(!normalized);
    // Vector should be left untouched
    CTEST_ASSERT(ez::math::isEqual(vector[0], static_cast<T>(0)));
    CTEST_ASSERT(ez::math::isEqual(vector[1], static_cast<T>(0)));
    CTEST_ASSERT(ez::math::isEqual(vector[2], static_cast<T>(0)));
    return true;
}

template <typename T>
bool TestEzVecN_Normalized() {
    ez::math::vecN<T> source{static_cast<T>(3), static_cast<T>(4)};
    ez::math::vecN<T> result = source.normalized();
    // Result has unit length
    CTEST_ASSERT(ez::math::isEqual(result.length(), static_cast<T>(1), static_cast<T>(1e-6)));
    // Source is untouched
    CTEST_ASSERT(ez::math::isEqual(source[0], static_cast<T>(3)));
    CTEST_ASSERT(ez::math::isEqual(source[1], static_cast<T>(4)));
    return true;
}
